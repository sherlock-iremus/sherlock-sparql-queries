...
UNION
{
  ${itemsDirectlyIndexed(projectCode, collectionUri, search)}
}
}`
}
...
WHERE {
    ...
    (?item ?score ?lit ?gr ?prop) text:query ("texte:${search}") .
    	?item crm:P1_is_identified_by|crm:P102_has_title|skos:prefLabel|crm:P48_has_preferred_identifier ?label .
    	?item ?p ?label .
...
WHERE {
  ...
  GRAPH ?g_meta {
    ...
    ?p177_e42 crm:P2_has_type <http://data-iremus.huma-num.fr/id/5372610b-88b1-4949-b87a-1e5102bf2fb7> .
    FILTER(isLiteral(?p177_label))
    FILTER(STRSTARTS(?project_code, "aam::"))
  }
}`
